...
use interfaces::{logger::error, models::{DsmSets, MergedSet, StorableMerged}, paths::write_to_file};

use super::{Localable, read_dsm, MERGED_SET, RAW_SET};
...
impl Localable for MergedSet {
    fn ledge(&self, ledger_directory: &std::path::Path) -> anyhow::Result<()> {
        ...
        let merged_file = format!("{}-v{}.json", self.get_name(), self.get_version());
        let merged_dir = ledger_directory.join(MERGED_SET);
        let merged_path = merged_dir.join(&merged_file);
        if merged_path.exists() {
            return error(format!("Merged set '{} v{}' already exists", self.get_name(), self.get_version()))
        }
        if ledger_directory.join(RAW_SET).join(merged_file).exists() {
            return error(format!("Raw set already exists with '{} v{}'", self.get_name(), self.get_version()))
        }
